...
let private updateItem item =
    ...
    | "Aged Brie" ->
        ...
        let qualityIncrease = if newSellIn < 0 then 2 else 1

        { item with
            SellIn = newSellIn
            Quality = clampQuality (item.Quality + qualityIncrease) }
    | "Backstage passes to a TAFKAL80ETC concert" ->
        ...
        { item with
            SellIn = newSellIn
            Quality = clampQuality newQuality }
    | name when name.StartsWith("Conjured") ->
        let newSellIn = item.SellIn - 1
        let degradation = if newSellIn < 0 then 4 else 2
    ...
    | _ ->
        let newSellIn = item.SellIn - 1
        let degradation = if newSellIn < 0 then 2 else 1

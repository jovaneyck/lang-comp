...
[<Fact>]
let ``Zombie cells stay zombie with zero live neighbors`` () =
    let input = grid [ [ Z ] ]

    tickGrid input |> should equal input

[<Fact>]
let ``Zombie cells stay zombie with three live neighbors`` () =
    let input =
        grid [ [ X; O; X ]
               [ O; Z; O ]
               [ X; O; O ] ]
...
[<Fact>]
let ``Zombie cells stay zombie with four or more live neighbors`` () =
    let input =
        grid [ [ X; O; X ]
               [ O; Z; O ]
               [ X; O; X ] ]
...
[<Fact>]
let ``Live cell with four or more zombie neighbors dies`` () =
    let input =
        grid [ [ Z; O; Z ]
               [ O; X; O ]
               [ Z; O; Z ] ]

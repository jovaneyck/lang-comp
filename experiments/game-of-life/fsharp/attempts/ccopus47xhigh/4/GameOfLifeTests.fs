...
[<Fact>]
let ``Zombie with zero live neighbors stays a zombie`` () =
    let input =
        grid [ [ O; O; O ]
               [ O; Z; O ]
               [ O; O; O ] ]
...
[<Fact>]
let ``Zombie with three live neighbors stays a zombie`` () =
    let input =
        grid [ [ X; X; X ]
               [ O; Z; O ]
               [ O; O; O ] ]
...
[<Fact>]
let ``Zombie surrounded by live neighbors stays a zombie`` () =
    let input =
        grid [ [ X; X; X ]
               [ X; Z; X ]
               [ X; X; X ] ]

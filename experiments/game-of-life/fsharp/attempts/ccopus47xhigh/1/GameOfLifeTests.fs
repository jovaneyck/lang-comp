...
let ``Zombie with three live neighbors stays zombie`` () =
    ...
    cellAt result { Row = 1; Col = 1 } |> should equal (Some Zombie)
...
let ``Dead cell with three zombie neighbors becomes alive`` () =
    ...
    let result = tickGrid input
    cellAt result { Row = 1; Col = 1 } |> should equal (Some Alive)
...
[<Fact>]
let ``Live cell surrounded by four zombies dies`` () =
    let input =
        grid [ [ O; Z; O ]
               [ Z; X; Z ]
               [ O; Z; O ] ]
    ...
    let result = tickGrid input
    cellAt result { Row = 1; Col = 1 } |> should equal (Some Dead)

...
[<Fact>]
let ``Zombie with four or more live neighbors stays zombie`` () =
    let input =
        grid [ [ X; X; X ]
               [ X; Z; X ]
               [ O; O; O ] ]

    let after = tickGrid input
    cellAt after { Row = 1; Col = 1 } |> should equal (Some Zombie)
...
let ``Dead cell with three zombie neighbors becomes alive`` () =
    ...
    let after = tickGrid input
    cellAt after { Row = 1; Col = 1 } |> should equal (Some Alive)
...
let ``Live cell surrounded by zombies dies from overcrowding`` () =
    ...
    let after = tickGrid input
    cellAt after { Row = 1; Col = 1 } |> should equal (Some Dead)

[<Fact>]
let ``Mixed grid with zombies after one tick`` () =
    let input =
